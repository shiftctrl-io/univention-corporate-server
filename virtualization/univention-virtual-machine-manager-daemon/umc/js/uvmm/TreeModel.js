...
define([
	...
	return declare('umc.modules.uvmm.TreeModel', null, {
		...
		_getGroupNameLabel: function(groupname) {
			...
			label = '<span class="tree-description">' + label + '</span>';
		...
		_getRessourceNodeLabel: function(node) {
			...
			var cpu = this._formatPercent(node.cpuUsage * 100);
			var mem = this._formatPercent(node.memUsed / node.memAvailable * 100);
