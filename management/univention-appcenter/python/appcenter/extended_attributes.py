...
def get_extended_attributes(app):
	...
	for section in parser.sections():
		...
		if kwargs.get('type') == 'ObjectClass':
			...
			options_default[kwargs.get('option_name', '%sUser' % (app.id,))] = kwargs.get('default')
		...
		elif kwargs.get('type') == 'ExtendedOption':
			attribute_logger.info('Ignore settings extended option %s in UCS < 4.4' % (section,))
			options_default[kwargs.get('name')] = kwargs.get('default')
		else:  # ignore, so that it is extensible for the future :-)
			attribute_logger.info('Unknown attribute type for section %s: %r' % (section, kwargs.get('type')))
	...
	if app.generic_user_activation:
		...
		if attribute_name:
			...
			attr.disable_web = False

	for attribute in attributes:
		for option in attribute.options:
			if option in options_default and attribute.udm_syntax in ('AppActivatedBoolean', 'AppActivatedTrue'):
				attribute.default = options_default[option]
				attribute.disable_web = False
		attribute.options = []
		if attribute.udm_syntax == 'AppActivatedBoolean':
			attribute.udm_syntax = 'boolean'
		if attribute.udm_syntax == 'AppActivatedTrue':
			attribute.udm_syntax = 'TrueFalseUp'
...
def create_extended_attribute(attribute, app, layout_position, lo, pos):
	...
	if attribute.default:
		attrs['default'] = str(int(attribute.default))
